...
func exec(runner runner, builder dat.Builder) (*dat.Result, error) {
	...
	return &dat.Result{RowsAffected: result.RowsAffected()}, nil
}

// Query delegates to the internal runner's Query.
func query(runner runner, builder dat.Builder) (*pgx.Rows, error) {
	fullSQL, args, err := builder.Interpolate()
	if err != nil {
		return nil, err
	}
...
func queryStruct(runner runner, builder dat.Builder, dest interface{}) error {
	...
	columns := getColumns(rows.FieldDescriptions())
...
func getColumns(fieldDescriptions []pgx.FieldDescription) []string {
	lenFD := len(fieldDescriptions)
	result := make([]string, lenFD)
