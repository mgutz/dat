...
func (m *Mapper) TypeMap(t reflect.Type) *FieldMeta {
	...
	info, ok := m.cache[t]
	if !ok {
		info = getMapping(t, m.tagName, m.mapFunc, m.tagMapFunc)
		m.cache[t] = info
	}
	m.mutex.Unlock()
	return info
...
func (m *Mapper) FieldsByName(v reflect.Value, names []string) []reflect.Value {
	...
	for _, name := range names {
		traversal, ok := nm[name]
		if !ok {
			panic(`db:"` + name + `" is missing on type ` + v.Type().Name())
...
func getMapping(t reflect.Type, tagName string, mapFunc, tagMapFunc func(string) string) *FieldMeta {
	queue := []typeQueue{}
